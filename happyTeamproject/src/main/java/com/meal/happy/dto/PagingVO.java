...
public class PagingVO {
	...
	private int onePageNumCount=10;//한페이지 당 페이지 갯수
	...
	public void setTotalRecord(int totalRecord) {
		...
		if(nowPage==totalPage) {
			if(totalRecord%onePageRecord==0){//나머지가 0일 경우에는 lastPageRecord <-onePageRecord
				lastPageRecord = onePageRecord;
			}else{//나머지가 있을 경우 레코드수를 lastPageRecord
